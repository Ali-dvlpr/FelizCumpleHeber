...
function startGiftAnimation() {
    ...
    const interval = setInterval(function() {
        ...
        // Colores de plasma y tecnología
        confetti(Object.assign({}, defaults, { particleCount, origin: { x: randomInRange(0.1, 0.3), y: Math.random() - 0.2 }, colors: ['#4cc9f0', '#ffffff', '#4361ee'] }));
        confetti(Object.assign({}, defaults, { particleCount, origin: { x: randomInRange(0.7, 0.9), y: Math.random() - 0.2 }, colors: ['#1e40af', '#4cc9f0', '#f8fafc'] }));
    }, 250);
    ...
    setTimeout(() => {
        ...
        void animatedText.offsetWidth; // Reflow
        
        animatedText.classList.remove('opacity-0', 'scale-50');
        animatedText.classList.add('opacity-100', 'scale-100');

        setTimeout(() => {
            const headerText = document.getElementById('header-text');
            
            // Preparar el contenedor final
            mainContent.classList.remove('hidden');
            headerText.style.visibility = 'hidden'; // Ocultar totalmente el texto real para evitar sombras/fantasmas
            void mainContent.offsetWidth; // Reflow
            
            const startRect = animatedText.getBoundingClientRect();
            const targetRect = headerText.getBoundingClientRect();
            const scale = targetRect.width / startRect.width;
            
            // Cambiar a posicionamiento estático absoluto
            animatedText.style.transition = 'none';
            animatedText.style.top = startRect.top + 'px';
            animatedText.style.left = startRect.left + 'px';
            animatedText.style.transformOrigin = 'top left';
            animatedText.style.transform = 'translate(0px, 0px) scale(1)';
            
            animatedText.classList.remove('top-1/2', 'left-1/2', '-translate-x-1/2', '-translate-y-1/2', 'scale-100');
            
            void animatedText.offsetWidth; // Reflow
            
            // Iniciar animación hacia el objetivo
            animatedText.style.transition = 'transform 1s ease-in-out, opacity 1s ease-in-out';
            
            const translateX = targetRect.left - startRect.left;
            const translateY = targetRect.top - startRect.top;
            
            animatedText.style.transform = `translate(${translateX}px, ${translateY}px) scale(${scale})`;
            
            mainContent.classList.remove('opacity-0');
            mainContent.classList.add('opacity-100');
            
            setTimeout(() => {
                // Intercambiar suavemente
                headerText.style.opacity = '0';
                headerText.style.visibility = 'visible';
                void headerText.offsetWidth; // Reflow for transition
...
messagesData.forEach(data => {
    let shadowColor = '';
    if(data.accentColor === 'halo-ring') shadowColor = 'rgba(67, 97, 238, 0.15)';
    if(data.accentColor === 'halo-energy') shadowColor = 'rgba(76, 201, 240, 0.15)';
    if(data.accentColor === 'halo-accent') shadowColor = 'rgba(30, 64, 175, 0.15)';
    if(data.accentColor === 'halo-success') shadowColor = 'rgba(16, 185, 129, 0.15)';

    const cardHTML = `
        <div class="glass-card card-hover rounded-xl cursor-pointer hologram-effect relative group aspect-[3/4] flex flex-col justify-end p-4 md:p-6 overflow-hidden" 
             onclick="openModal(${data.id})">
            
            <img src="${data.image}" alt="" class="absolute inset-0 w-full h-full object-cover opacity-40 group-hover:opacity-95 mix-blend-luminosity group-hover:mix-blend-normal transition-all duration-700 z-0">
            
            <div class="absolute inset-0 bg-[radial-gradient(ellipse_at_center,_var(--tw-gradient-stops))] from-transparent via-halo-void/80 to-halo-void z-0 transition-opacity duration-300"></div>
            <div class="absolute inset-0 z-0 opacity-50" style="background: radial-gradient(circle at center, ${shadowColor} 0%, transparent 70%);"></div>

            <div class="relative z-10 w-full">
                <p class="text-xs font-mono tracking-widest text-${data.accentColor} mb-1 uppercase drop-shadow-md bg-halo-void/60 inline-block px-2 py-0.5 rounded border border-${data.accentColor}/30">${data.sender}</p>
                <h4 class="text-xl md:text-2xl font-display font-bold text-white leading-tight drop-shadow-lg uppercase">${data.title}</h4>
            </div>
        </div>
    `;
...
function openModal(id) {
    const data = messagesData.find(item => item.id === id);
    if (!data) return;
    currentActiveId = id;
    ...
    titleElement.className = `text-3xl md:text-4xl font-display font-bold text-white uppercase tracking-wide border-l-4 border-${data.accentColor} pl-4`;
...
function navigateModal(direction) {
    ...
    const currentIndex = messagesData.findIndex(item => item.id === currentActiveId);
    if (currentIndex === -1) return;
    
    let nextIndex = currentIndex + direction;
    if (nextIndex >= messagesData.length) nextIndex = 0;
    if (nextIndex < 0) nextIndex = messagesData.length - 1;

    // Transición de salida/entrada del contenido
    modalContent.classList.remove('scale-100');
    modalContent.classList.add('scale-95');
    
    setTimeout(() => {
        const data = messagesData[nextIndex];
...
function scrollBanner(direction) {
    ...
    // Resetear temporizador de 5 segundos
    initBannerTimer();
...
// Inicializar el escuchador de transiciones del slider
...
if (bannerSlider) {
    ...
    // Activar deslizado táctil en el carrusel
    addSwipeListener(bannerSlider.parentElement, () => scrollBanner(1), () => scrollBanner(-1));
}

// Activar deslizado táctil en el modal de cartas
const modalOverlayElement = document.getElementById('modalOverlay');
if (modalOverlayElement) {
    addSwipeListener(modalOverlayElement, () => navigateModal(1), () => navigateModal(-1));
}

function updateScrollIndicator() {
    const container = document.getElementById('modalMessageScrollable');
    ...
    // Verificar si el texto desborda el contenedor (necesita scroll)
    setTimeout(() => {
...
function handleModalScroll(element) {
    const indicator = document.getElementById('scrollIndicator');
    if (!indicator) return;
    
    // Ocultar si se scrollea hasta abajo (con umbral de 20px)
